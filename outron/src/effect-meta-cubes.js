class MetaCubesEffect {
  constructor({ id, canvas, clear = true }) {
    ...
    this.programInfo = new ProgramInfo({
      ...
      fragmentShaderId: "metacubes-fragment-shader",
    ...
    this.noise = [];
    for (let i = 0; i < 12; i++) {
      this.noise.push(metaCubeNoise(i));
      console.log(metaCubeNoise(i));
    }
    for (let i = 0; i < 12; i++) {
      this.noise.push(metaCubeNoise(i + 18));
      console.log(metaCubeNoise(i + 18));
    }
  }
  ...
  render(now = 0) {
    ...
    const rayOrigin = { x: 0, y: 3, z: -4 };
    rayOrigin.x *= c;
    rayOrigin.x *= -s;
    rayOrigin.y *= s;
    rayOrigin.y *= c;

    gl.uniform3f(this.programInfo.uniforms.lightPosition, 0, 9, 0);
    gl.uniform3f(
